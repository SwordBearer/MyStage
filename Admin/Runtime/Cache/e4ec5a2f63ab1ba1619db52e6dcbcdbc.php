			...
			<table class="table table-condensed table-hover" >
				...
				<tbody>
					<?php if(is_array($allBlogs)): $i = 0; $__LIST__ = $allBlogs;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$blog): $mod = ($i % 2 );++$i;?><tr>
							...
							<td>
								<a href="<?php echo U(recoveryBlog);?>/blogid/<?php echo ($blog["id"]); ?>"class="btn btn-small btn-warning">恢复</a>
								<input type="button" class="btn btn-small btn-danger"value="删除" onClick="deleteBlog(<?php echo ($blog["id"]); ?>)" ></td>
